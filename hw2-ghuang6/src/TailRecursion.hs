...
assoc def _ [] = def
assoc def key (kv:kvs)
  | key == fst kv = snd kv
  | otherwise     = assoc def key kvs

-- below is a way to do it without fst and snd
...
-- cleaner ways to write the above function call..
-- wwhile (\x -> (x^3 < 100, x^3)) 2
-- wwhile f 2 where f x = (x^3 < 100, x^3)
-- 512

-- note: wwhile is very similar to Haskell's "until" function
-- until (> 100) (^3) 2
-- 512
...
-- my second working attempt (only call f n once):
wwhile :: (a -> (Bool, a)) -> a -> a
wwhile f n
  | first == False  = second
  | otherwise       = wwhile f second
  where (first, second) = f n

-- using let statement instead of where statement
-- wwhile :: (a -> (Bool, a)) -> a -> a
-- wwhile f n = let (first, second) = f n in
--   if first then wwhile f second else second

-- my third working attempt is uncommented (rewrote second attempt with different syntax)
...
-- Below is a tail recursive fixpointL
-- fixpointL f x  = helper [] f x where
...
fixpointW f x = wwhile wwf x
 where
   ...
   wwf x      = (f x /= x, f x)
   -- alternate way below:
   -- wwf x   = if (f x == x) then (False, x) else (True, f x)
   -- another alternate way (without editing lefthand side of equals) using lambda expression:
   -- wwf     = \x = (f x /= x, f x)
