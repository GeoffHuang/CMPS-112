...
-- NOTE: run make (not "stack test") if your images aren't coming out properly
...
-- part (d)
exprToString (Max e1 e2)          = "max(" ++ exprToString e1 ++ "," ++ exprToString e2 ++ ")"
exprToString (ITE e1 e2 e3)       = "(" ++ exprToString e1 ++ ">0?" ++ exprToString e2 ++ ":" ++ exprToString e3 ++ ")"
...
-- part (d)
eval x y (Max e1 e2)          = if eval x y e1 > eval x y e2 then eval x y e1 else eval x y e2
eval x y (ITE e1 e2 e3)       = if eval x y e1 > 0           then eval x y e2 else eval x y e3

-- alternate syntax below (case expression):
-- eval x y e = 
--   case e of
--     VarX                 -> x
--     VarY                 -> y
...
--     -- part (d)
--     (Max e1 e2)          -> if eval x y e1 > eval x y e2 then eval x y e1 else eval x y e2
--     (ITE e1 e2 e3)       -> if eval x y e1 > 0           then eval x y e2 else eval x y e3
...
build d
  | r == 1    = Sine    (build (d-1))
  | r == 6    = Average (build (d-1)) (build (d-1))
  | r == 7    = Times   (build (d-1)) (build (d-1))
  | r == 8    = Thresh  (build (d-1)) (build (d-1)) (build (d-1)) (build (d-1))
  | r == 9    = Max     (build (d-1)) (build (d-1))
  | r == 10   = ITE     (build (d-1)) (build (d-1)) (build (d-1))
  | otherwise = Cosine  (build (d-1))
  -- note: r == 0 or 2-5 results in Cosine  (build (d-1))
  -- don't ask me why I did it this way, but I got some cool pictures so I cbf to change it
  where
    r         = rand 11
